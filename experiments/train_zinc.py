...
def main():
    ...
    # model_simclr = SimpleGNN(in_features=input_size,    # in_features = 進入模型的節點特徵維度
    #               hidden_dim=args.dim_hidden,  # hidden_dim = 每層 GCN 層的隱藏維度
    #               out_features=1,              # out_features = 最終輸出的維度 (1 是分類任務中的類別數量)
    #               num_gc_layers=args.num_layers)
    ...
    for epoch in range(args.epochs):
        ...
        val_loss,_ = eval_epoch(model, model_simclr, val_loader, criterion, args.use_cuda, split='Val', unsupervised=args.unsupervised)
        test_loss,_ = eval_epoch(model, model_simclr, test_loader, criterion, args.use_cuda, split='Test', unsupervised=args.unsupervised)
    ...
    print()
    print("Testing...")
    test_loss, test_mse_loss = eval_epoch(model, model_simclr, test_loader, criterion, args.use_cuda, split='Test')
